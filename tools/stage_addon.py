...
def main() -> int:
    ...
    installed_toc_name = toc.name if args.keep_toc_name else f"{args.addon_name}.toc"
    existing_toc = destination / installed_toc_name
    if not existing_toc.is_file() and args.keep_toc_name:
        existing_toc = destination / f"{args.addon_name}.toc"
    if not existing_toc.is_file() and not args.keep_toc_name:
        existing_toc = destination / f"{args.addon_name}_Camelot.toc"
    dev_version = staged_version(toc, existing_toc, args.dev)
